...
#define ROM_DIRECTORY_TS ROM_DIRECTORY "/"
...
char* relative_rom_path_to_absolute(char* path) {
    ...
    if (strncmp(path, ROM_DIRECTORY_TS, strlen(ROM_DIRECTORY_TS)) == 0) {
        // Already an absolute path
        return strdup(path);
    }

    // Add ROM_DIRECTORY prefix
    char* result = malloc(strlen(ROM_DIRECTORY_TS) + strlen(path) + 1);
    if (!result) {
        log_message(LOG_ERROR, "Failed to allocate memory for absolute path");
        return NULL;
    }

    strcpy(result, ROM_DIRECTORY_TS);
    strcat(result, path);
